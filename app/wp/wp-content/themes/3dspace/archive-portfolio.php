<?get_header();?>   
    ...
    <div class="row title-page">
        <div class="columns large-12">
            <h1><? if(get_locale() == 'uk') {?>Всі проекти<? }elseif(get_locale() == 'ru_RU') { ?>Все проекты<? } ?></h1>
        </div>
    </div>
    ...
    <div class="row content-holder portfolio">
       <?php if(function_exists('bcn_display') && !is_front_page()) { ?>
        ...
        <div class="change-class">
            <a href="/portfolio_category/dyzajn-ynterera/';" class="class-1"><? if(get_locale() == 'uk') {?>Дизайн інтер'єру<? }elseif(get_locale() == 'ru_RU') { ?>Дизайн интерьера<? } ?></a>
            <a href="/portfolio_category/dyzajn-eksterera/" class="class-2"><? if(get_locale() == 'uk') {?>Дизайн екстер'єру<? }elseif(get_locale() == 'ru_RU') { ?>Дизайн экстерьера<? } ?></a>
            <a href="/portfolio_category/proekty-domov/" class="class-3"><? if(get_locale() == 'uk') {?>Проекти будинків<? }elseif(get_locale() == 'ru_RU') { ?>Проекты домов<? } ?></a>
            <a href="/portfolio/" class="all active"><? if(get_locale() == 'uk') {?>Всі проекти<? }elseif(get_locale() == 'ru_RU') { ?>Все проекты<? } ?></a>
        </div>
